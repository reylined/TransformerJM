...
def simulate_JM_nonPH(I, obstime, miss_rate=0.1, opt="none", seed=None):
    ...
    Ti[:] = np.NaN
    for i in range(0,I):
        Ti[i] = optimize.brentq(lambda xi: U[i]-CHF(xi), 0, 500)
    ...
    for i in range(0,3):
        ...
        Y_pred[:,i] = eta_long[:,i] + betat[i]*subj_obstime + long_err[:,i]
